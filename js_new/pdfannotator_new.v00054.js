(function () {
    ...
    function computeAnchorPageFromScroll(viewer, scrollTop) {
        var st = Math.max(0, Number(scrollTop) || 0);
        var best = 1;
        var pages = viewer.querySelectorAll('.page[data-page-number]');
        pages.forEach(function (el) {
            var pn = parseInt(el.getAttribute('data-page-number') || '0', 10);
            if (!pn) {
                return;
            }
            if (el.offsetTop <= st + 2) {
                best = pn;
            }
        });
        return best;
    ...
    function getPageContentScrollMetrics(pageNum) {
        var pageEl = getPageElement(pageNum);
        if (!pageEl) {
            return null;
        }
        ...
        var contentTop = pageTop + canvasTop;
        var contentHeight = canvas && canvas.offsetHeight ? Math.max(1, canvas.offsetHeight) : Math.max(1, pageEl.offsetHeight || 1);
        return {
            contentTop: contentTop,
            contentHeight: contentHeight,
            pageTop: pageTop,
            canvasTop: canvasTop,
            pageHeight: pageEl.offsetHeight || null,
            canvasHeight: canvas ? (canvas.offsetHeight || null) : null
        };
    }
    ...
    function buildSavedPosition(pageNo, scrollTop) {
        ...
        var anchorPage = viewer ? computeAnchorPageFromScroll(viewer, top) : Math.max(1, parseInt(pageNo || 1, 10) || 1);
        if (state.pdf && state.pdf.numPages) {
            anchorPage = Math.min(state.pdf.numPages, Math.max(1, anchorPage));
        }
        var ratio = null;
        var metrics = getPageContentScrollMetrics(anchorPage);
        var actualPageOffsetTop = null;
        var actualPageOffsetHeight = null;
        var actualPageTop = null;
        var actualCanvasTop = null;
        var actualPageHeight = null;
        var actualCanvasHeight = null;
        if (metrics) {
            actualPageOffsetTop = metrics.contentTop;
            actualPageOffsetHeight = metrics.contentHeight;
            actualPageTop = Number.isFinite(metrics.pageTop) ? metrics.pageTop : null;
            actualCanvasTop = Number.isFinite(metrics.canvasTop) ? metrics.canvasTop : null;
            actualPageHeight = Number.isFinite(metrics.pageHeight) ? metrics.pageHeight : null;
            actualCanvasHeight = Number.isFinite(metrics.canvasHeight) ? metrics.canvasHeight : null;
            ratio = clampNumber((top - actualPageOffsetTop) / actualPageOffsetHeight, 0, 1);
        }
        ...
        return {
            ...
            scale: state.scale,
            signature: signature,
            pageOffsetTop: actualPageOffsetTop,
            pageOffsetHeight: actualPageOffsetHeight,
            pageTop: actualPageTop,
            canvasTop: actualCanvasTop,
    ...
    function normalizePendingTargetPage(pending) {
        if (!pending) {
            return 1;
        }
        var p = Math.max(1, parseInt(pending.page, 10) || 1);
        if (state.pdf && state.pdf.numPages) {
            p = Math.min(state.pdf.numPages, p);
        }
        return p;
    }

    function applyScrollFromRestorePending(options) {
        var opts = options || {};
        ...
        var restoreCurrentPage = computeAnchorPageFromScroll(viewer, viewer.scrollTop);
        if (!viewer) {
            return;
        }
        var pageNum = normalizePendingTargetPage(pending);
        var pageEl = getPageElement(pageNum);
        if (!pageEl) {
            if (Number.isFinite(pending.scrollFraction)) {
                var maxScrollEarly = Math.max(0, viewer.scrollHeight - viewer.clientHeight);
                viewer.scrollTop = Math.round(clampNumber(pending.scrollFraction, 0, 1) * maxScrollEarly);
                if (commit) {
        ...
        var pendingPageTop = Number.isFinite(pending.pageTop) ? pending.pageTop : null;
        var pendingCanvasTop = Number.isFinite(pending.canvasTop) ? pending.canvasTop : null;
        var pendingPageHeight = Number.isFinite(pending.pageHeight) ? pending.pageHeight : null;
        var pendingCanvasHeight = Number.isFinite(pending.canvasHeight) ? pending.canvasHeight : null;
        var currentTop = metrics && Number.isFinite(metrics.contentTop) ? metrics.contentTop : null;
        var currentHeight = metrics && Number.isFinite(metrics.contentHeight) ? metrics.contentHeight : null;
        var currentPageTop = metrics && Number.isFinite(metrics.pageTop) ? metrics.pageTop : null;
        var currentCanvasTop = metrics && Number.isFinite(metrics.canvasTop) ? metrics.canvasTop : null;
        var currentPageHeight = metrics && Number.isFinite(metrics.pageHeight) ? metrics.pageHeight : null;
        var currentCanvasHeight = metrics && Number.isFinite(metrics.canvasHeight) ? metrics.canvasHeight : null;

        if (useRatio && pending.captureSignature && currentSignature !== pending.captureSignature) {
            useRatio = false;
        }
        var pageHeightDeltaRatio = null;
        if (useRatio) {
            if (!Number.isFinite(currentHeight)) {
                useRatio = false;
            } else if (pendingHeight == null) {
                // page height snapshot not yet available; still trust ratio when current metrics are present
                useRatio = true;
            } else {
                pageHeightDeltaRatio = Math.abs(currentHeight - pendingHeight) / Math.max(1, pendingHeight);
            }
        }

        var targetTop;
        if (metrics && useRatio && ratio !== null) {
            targetTop = metrics.contentTop + ratio * metrics.contentHeight;
        } else if (Number.isFinite(pending.scrollTop)) {
            targetTop = pending.scrollTop;
        } else if (Number.isFinite(pending.scrollFraction)) {
            var maxScrollFr = Math.max(0, viewer.scrollHeight - viewer.clientHeight);
            targetTop = clampNumber(pending.scrollFraction, 0, 1) * maxScrollFr;
        } else if (metrics) {
            targetTop = metrics.contentTop;
        } else {
            targetTop = pageEl.offsetTop;
        }
        var maxScroll = Math.max(0, viewer.scrollHeight - viewer.clientHeight);
        var appliedTop = Math.max(0, Math.min(maxScroll, Math.round(targetTop)));
        viewer.scrollTop = appliedTop;
        requestAnimationFrame(function () {
            requestAnimationFrame(function () {
                var viewerAfter = viewerEl();
                if (!viewerAfter) {
                    return;
                }
                var settleMetrics = getPageContentScrollMetrics(pageNum);
            });
        });
        if (commit) {
            state.restorePositionPending = null;
    ...
    function applyPendingSavedPosition(renderedPageNo, restoreBatchCaptured) {
        var pending = state.restorePositionPending;
        if (!pending) {
            return;
        }
        if (normalizePendingTargetPage(pending) !== Number(renderedPageNo)) {
            return;
        }
        if (restoreBatchCaptured != null && pending.batchId != null && pending.batchId !== restoreBatchCaptured) {
            return;
        }
        requestAnimationFrame(function () {
            requestAnimationFrame(function () {
                var pend = state.restorePositionPending;
                if (!pend) {
                    return;
                }
                if (normalizePendingTargetPage(pend) !== Number(renderedPageNo)) {
                    return;
                }
                if (restoreBatchCaptured != null && pend.batchId != null && pend.batchId !== restoreBatchCaptured) {
                    return;
                }
                applyScrollFromRestorePending({ commit: true });
            });
        });
    }
    ...
    function restoreScrollAfterTheatreLayoutIfPossible() {
        var pending = state.restorePositionPending;
        if (!pending || !Number.isFinite(pending.scrollFraction)) {
            return;
        }
        var viewer = viewerEl();
        if (!viewer) {
        ...
        var appliedTop = Math.round(clampNumber(pending.scrollFraction, 0, 1) * maxScroll);
        viewer.scrollTop = appliedTop;
    }
    ...
    function bindViewerScroll() {
        ...
        viewer.addEventListener('scroll', function () {
            ...
            if (state.__dbg574c2cRunId && dt > 0 && dt < 1500 && delta > 60 && (state.restorePositionPending || isTheatreLayoutTransitionBusy())) {
            }
    ...
    function rerenderAfterDisplayModeChange(delayMs, options) {
        ...
        state.displayModeRerenderTimer = setTimeout(function () {
            ...
            var viewer = viewerEl();
            if (viewer) {
                state.savedPosition = buildSavedPosition(getCurrentPage(true), viewer.scrollTop);
                state.restorePositionPending = {
                    ...
                    scrollFraction: state.savedPosition.scrollFraction,
                    scale: state.savedPosition.scale,
                    captureSignature: state.savedPosition.signature,
                    pageOffsetTop: state.savedPosition.pageOffsetTop,
                    pageOffsetHeight: state.savedPosition.pageOffsetHeight,
                    pageTop: state.savedPosition.pageTop,
                    canvasTop: state.savedPosition.canvasTop,
                    pageHeight: state.savedPosition.pageHeight,
                    canvasHeight: state.savedPosition.canvasHeight
                };
        ...
        state.displayModeRerenderTimer = setTimeout(function () {
            ...
            if (saved) {
                ...
                state.restorePositionPending = {
                    ...
                    scale: state.scale,
                    batchId: state.restoreBatchId,
                    captureSignature: savedFromPreToggle ? null : saved.signature,
                    pageOffsetTop: saved.pageOffsetTop,
                    pageOffsetHeight: saved.pageOffsetHeight,
                    pageTop: saved.pageTop,
                    canvasTop: saved.canvasTop,
    ...
    function buildShoelaceToolbar() {
        ...
        shell.innerHTML = [
            ...
            '<label title="Text size"><span class="tl-style-glyph tl-style-glyph--textsize"><svg viewBox="0 0 12 20" width="12" height="19" aria-hidden="true" focusable="false"><path fill="currentColor" d="M6 0.005 L9.70 3.829 V4.505 H7.26 V15.495 H9.70 V16.171 L6 19.995 L2.30 16.171 V15.495 H4.74 V4.505 H2.30 V3.829 Z"/></svg></span></label>',
    ...
    function reflowPdfForScaleChange() {
        ...
        if (state.savedPosition && Number.isFinite(state.savedPosition.page)) {
            ...
            state.restorePositionPending = {
                ...
                scrollFraction: Number.isFinite(state.savedPosition.scrollFraction) ? state.savedPosition.scrollFraction : null,
                scale: Number.isFinite(state.savedPosition.scale) ? state.savedPosition.scale : state.scale,
                captureSignature: state.savedPosition.captureSignature || null,
                pageOffsetTop: Number.isFinite(state.savedPosition.pageOffsetTop) ? state.savedPosition.pageOffsetTop : null,
                pageOffsetHeight: Number.isFinite(state.savedPosition.pageOffsetHeight) ? state.savedPosition.pageOffsetHeight : null,
                pageTop: Number.isFinite(state.savedPosition.pageTop) ? state.savedPosition.pageTop : null,
                canvasTop: Number.isFinite(state.savedPosition.canvasTop) ? state.savedPosition.canvasTop : null,
                pageHeight: Number.isFinite(state.savedPosition.pageHeight) ? state.savedPosition.pageHeight : null,
                canvasHeight: Number.isFinite(state.savedPosition.canvasHeight) ? state.savedPosition.canvasHeight : null
            };
        }
        if (viewer) {
    ...
    function removeStaleHtmlAnnotationElementsFromPage(pageContainer) {
        if (!pageContainer) {
            return;
        }
        var editingId = null;
    ...
    function purgeStaleHtmlAnnotationLayersForVisiblePages() {
        if (!state.pdf) {
            return;
        }
        var range = getVisiblePageRange();
        var i;
        for (i = range.from; i <= range.to; i++) {
            removeStaleHtmlAnnotationElementsFromPage(getPageElement(i));
        }
    }
    ...
    function renderPage(pageNumber) {
        ...
        return state.pdf.getPage(pageNumber).then(function (page) {
            ...
            return pdfRenderTask.promise.then(function () {
                ...
                applyPendingSavedPosition(pageNumber, restoreBatchCaptured);
    ...
    function textboxPaddingPx(fontSizePx) {
        return Math.max(0, Math.round(0.4 * (Number(fontSizePx) || 0)));
    }

    function textboxMeasureParent(pageElement) {
        if (pageElement) {
            return pageElement;
        }
        var viewer = viewerEl();
        if (viewer) {
            var page = viewer.querySelector('.page');
            if (page) {
                return page;
            }
        }
    ...
    function measureTextboxBlock(pageElement, fontSizePx, fontFamily, content, options) {
        options = options || {};
        ...
        probe.style.cssText = 'position:absolute;left:-9999px;top:0;visibility:hidden;box-sizing:content-box;margin:0;border:none;padding:0;pointer-events:none;';
        probe.style.fontSize = fs + 'px';
        probe.style.fontFamily = (fontFamily || 'Open Sans') + ', sans-serif';
        probe.style.lineHeight = '1.25';
        probe.style.whiteSpace = whiteSpace;
        probe.style.wordBreak = whiteSpace === 'pre-wrap' ? 'break-word' : 'normal';
        if (innerWidth > 0) {
            probe.style.width = Math.max(1, Math.round(innerWidth)) + 'px';
        }
        probe.textContent = (content && String(content).length) ? String(content) : ' ';
        parent.appendChild(probe);
        var textHeight = probe.offsetHeight || Math.max(1, Math.round(fs * 1.25));
        var textWidth = probe.offsetWidth || 0;
        if (probe.parentNode) {
    ...
    function computeWrappedLineCount(pageElement, text, widthPx, paddingX, paddingY, fontSizePx, fontFamily) {
        try {
            ...
            var padX = Math.max(0, Math.round(paddingX));
            var innerWidth = Math.max(1, Math.round(widthPx - 2 * padX));
            var measured = measureTextboxBlock(pageElement, fontSizePx, fontFamily, text, {
                whiteSpace: 'pre-wrap',
                innerWidth: innerWidth
            });
            var lh = Math.max(1, Math.round((Number(fontSizePx) || 14) * 1.25));
            return Math.max(1, Math.ceil(measured.textHeight / lh + 0.15));
        } catch (e) {
            return 1;
        }
    }
    ...
    function textboxUnscaledXAlignEditorText(editor, fontSizeUnscaled, scale) {
        ...
        var padLpx = textboxPaddingPx(fsPx);
        var edPad = 0;
        try {
    ...
    function applyTextboxLabelLayout(pageNumber, labelEl, annotation, scale) {
        ...
        layoutTextboxLabelEl(labelEl, annotation, scale, pageElement);
    }
    ...
    function showTextboxEditor(pageNumber, annotationData) {
        ...
        var displayPad = textboxPaddingPx(displayFontSize);
        ...
        editor.style.padding = displayPad + 'px';
        editor.style.boxSizing = 'border-box';
        editor.style.width = Math.max(80, annotationData.width * state.scale) + 'px';
        editor.style.height = Math.max(12, annotationData.height * state.scale) + 'px';
        ...
        function resizeEditorToContent() {
            ...
            var h = Math.max(12, Math.ceil(tmp.height * scale));
    ...
    function showNewTextboxEditor(pageNumber, pointerX, pointerY) {
        ...
        var displayFontSize = Math.max(1, Math.round(editorFontSize * (state.scale || 1)));
        var displayPad = textboxPaddingPx(displayFontSize);
        var _emptyBox = { size: editorFontSize, font: editorFontFamily, content: ' ' };
        layoutTextboxBoxUnscaled(pageElement, _emptyBox);
        ...
        if (page !== 1) {
            ...
            if (stored && typeof stored.page === 'number' && stored.page >= 1) {
                ...
                state.savedPosition = {
                    ...
                    scrollFraction: Number.isFinite(stored.scrollFraction) ? clampNumber(stored.scrollFraction, 0, 1) : null,
                    scale: state.scale,
                    captureSignature: typeof stored.captureSignature === 'string' ? stored.captureSignature : null,
                    pageOffsetTop: Number.isFinite(stored.pageOffsetTop) ? stored.pageOffsetTop : null,
                    pageOffsetHeight: Number.isFinite(stored.pageOffsetHeight) ? stored.pageOffsetHeight : null,
                    pageTop: Number.isFinite(stored.pageTop) ? stored.pageTop : null,
                    canvasTop: Number.isFinite(stored.canvasTop) ? stored.canvasTop : null,
                    pageHeight: Number.isFinite(stored.pageHeight) ? stored.pageHeight : null,
                    canvasHeight: Number.isFinite(stored.canvasHeight) ? stored.canvasHeight : null
                };
            } else {
                state.initialPage = 1;
                state.savedPosition = null;
